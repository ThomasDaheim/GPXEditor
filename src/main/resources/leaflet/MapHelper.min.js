myMap.options.worldCopyJump=!0;myMap.options.zoomAnimation=!1;myMap.options.preferCanvas=!0;function disableCntrlDrag(a){a.originalEvent.ctrlKey&&(myMap.dragging.disable(),myMap.boxZoom.disable())}function enableCntrlDrag(a){a.originalEvent.ctrlKey&&(myMap.dragging.enable(),myMap.boxZoom.enable())}myMap.on("mousedown",disableCntrlDrag);myMap.on("mouseup",enableCntrlDrag);function mapViewChanged(a){var b=getMapBounds();jscallback.mapViewChanged(a.type,b[0],b[1],b[2],b[3])}myMap.on("zoomend",mapViewChanged);
myMap.on("moveend",mapViewChanged);myMap.on("resize",mapViewChanged);var mapBounds;function setMapBounds(a,b,c,e,d){mapBounds=[[a,c],[b,e]];d&&doSetMapBounds(mapBounds)}function doSetMapBounds(a){"undefined"!==typeof a&&myMap.fitBounds(a)}function getMapBounds(){var a=myMap.getBounds();return[a.getSouthWest().lat,a.getSouthWest().lng,a.getNorthEast().lat,a.getNorthEast().lng]}function updateMarkerIcon(a,b){window[a].setIcon(window[b])}
function updateMarkerColor(a,b){window[a].setStyle({color:b,weight:2})}function highlightMarker(a){window[a].valueOf()._icon.style.backgroundColor="red"}function unlightMarker(a){window[a].valueOf()._icon.style.backgroundColor="transparent"}function updateMarkerLocation(a,b,c){a=window[a];b=new L.LatLng(b,c);a.setLatLng(b)}function addClickToLayer(a,b,c){window[a].on("click",function(e){jscallback.selectMarker(a,b,c,e.originalEvent.shiftKey)})}
