...
c.options.dateFormatFn(a);c._time.innerHTML=c.options.timeFormatFn(a)});return this._container}});b.Playback.PlayControl=b.Control.extend({options:{position:"bottomright"},initialize:function(a){this.playback=a},onAdd:function(a){this._container=b.DomUtil.create("div","leaflet-control-layers leaflet-control-layers-expanded");var c=this,d=this.playback;d.setSpeed(100);a=b.DomUtil.create("div","playControl",this._container);this._button=b.DomUtil.create("button","",a);this._button.innerHTML="Play";
a=b.DomEvent.stopPropagation;b.DomEvent.on(this._button,"click",a).on(this._button,"mousedown",a).on(this._button,"dblclick",a).on(this._button,"click",b.DomEvent.preventDefault).on(this._button,"click",function(){d.isPlaying()?(d.stop(),c._button.innerHTML="Play"):(d.start(),c._button.innerHTML="Stop")},this);return this._container}});b.Playback.SliderControl=b.Control.extend({options:{position:"bottomleft"},initialize:function(a){this.playback=a},onAdd:function(a){function c(a){e.setCursor(Number(a.target.value))}
